 ...
 *
 *     fun deactivate() {
 *         require(status == UserStatus.ACTIVE) { "User must be active to deactivate" }
 *
 *         status = UserStatus.INACTIVE
 *
 *         recordEvent(UserDeactivatedEvent(
 *             aggregateId = id.value,
 *             userId = id.value,
 ...
 *
 * // Publish events (typically through Transactional Outbox)
 ...
 */
public abstract class AggregateRoot<ID> : com.melsardes.libraries.structuskotlin.domain.Entity<ID>() {
    /**
     ...
     */
    private val _domainEvents: MutableList<com.melsardes.libraries.structuskotlin.domain.events.DomainEvent> = mutableListOf()
     ...
     */
    public val domainEvents: List<com.melsardes.libraries.structuskotlin.domain.events.DomainEvent>
     ...
     */
    protected fun recordEvent(event: com.melsardes.libraries.structuskotlin.domain.events.DomainEvent) {
     ...
     */
    public fun eventCount(): Int = _domainEvents.size
     ...
     * Should be set by the infrastructure layer when first persisting the aggregate.
     */
    public open var createdBy: String? = null
        protected set
     ...
     */
    public open var updatedAt: kotlin.time.Instant? = null
        protected set
     ...
     * Should be updated by the infrastructure layer on every save operation.
     */
    public open var updatedBy: String? = null
        protected set
     ...
     */
    public open var deletedAt: kotlin.time.Instant? = null
        protected set

    /**
     * Identifier of the user/system that soft-deleted this aggregate.
     */
    public open var deletedBy: String? = null
        protected set
     ...
     */
    public open var version: Long = 0
        protected set
     ...
     */
    internal fun markAsCreated(by: String, at: kotlin.time.Instant = kotlin.time.Clock.System.now()) {
        createdAt = at
        createdBy = by
        updatedAt = at
        updatedBy = by
     ...
     */
    internal fun markAsUpdated(by: String, at: kotlin.time.Instant = kotlin.time.Clock.System.now()) {
        updatedAt = at
        updatedBy = by
    }
     ...
     */
    public fun softDelete(by: String, at: kotlin.time.Instant = kotlin.time.Clock.System.now()) {
     ...
     */
    public fun restore(by: String, at: kotlin.time.Instant = kotlin.time.Clock.System.now()) {
     ...
     */
    public fun isDeleted(): Boolean = deletedAt != null
